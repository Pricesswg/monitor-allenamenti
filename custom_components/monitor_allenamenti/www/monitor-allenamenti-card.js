  ...
  `}let wt=class extends dt{render(){const t=this.card.hass,e=this.card.monitorState,i=t.states["sensor.withings_peso"]?.state??"—",s=e?.points_total??0,r=e?.streak??0,a=e?.workouts??[],o=new Date,n=`${o.getFullYear()}-${String(o.getMonth()+1).padStart(2,"0")}`,l=a.filter(t=>t.date?.startsWith(n)).length,d=[...a].sort((t,e)=>e.date.localeCompare(t.date)).slice(0,5),c=(e?.weight_history??[]).map(t=>t.weight),p=c.length>=2?c[c.length-1]-c[Math.max(0,c.length-8)]:0,h=c.length>=2?$t(c.slice(-14),160,36):"";return V`
    ...
    `}};Ct.styles=xt,t([gt({attribute:!1,hasChanged:()=>!0})],Ct.prototype,"card",void 0),t([ut()],Ct.prototype,"_filter",void 0),Ct=t([pt("monitor-workouts")],Ct);const At=["7G","30G","3M","6M","1A"];function Mt(t){switch(t){case"7G":return 7;case"30G":return 30;case"3M":return 90;case"6M":return 180;case"1A":return 365}}function St(t,e){const i=t.states[e];return i&&"unknown"!==i.state&&"unavailable"!==i.state?i.state:null}let Et=class extends dt{constructor(){super(...arguments),this._range="30G"}render(){const t=this.card.hass,e=parseFloat(St(t,"sensor.withings_peso")||"0"),i=parseFloat(St(t,"sensor.withings_altezza")||"0"),s=parseFloat(St(t,"sensor.withings_obiettivo_di_peso")||"0"),r=parseFloat(St(t,"sensor.withings_passi_oggi")||"0"),a=parseFloat(St(t,"sensor.withings_calorie_attive_bruciate_oggi")||"0"),o=parseFloat(St(t,"sensor.withings_calorie_totali_bruciate_oggi")||"0"),n=parseFloat(St(t,"sensor.withings_distanza_percorsa_oggi")||"0"),l=parseFloat(St(t,"sensor.withings_attivita_intensa_oggi")||"0"),d=parseFloat(St(t,"sensor.withings_attivita_moderata_oggi")||"0"),c=parseFloat(St(t,"sensor.withings_attivita_leggera_oggi")||"0"),p=parseFloat(St(t,"sensor.withings_variazione_dell_elevazione_oggi")||"0"),h=i>0?e/(i*i):0,v=e-s,g=n/1e3,u=this.card.monitorState?.weight_history??[],m=Mt(this._range),x=new Date;x.setDate(x.getDate()-m);const f=x.toISOString().slice(0,10),_=u.filter(t=>t.date>=f),b=_.map(t=>t.weight),k=u.length>0?u[0].date:"";let $=0;if(k){const t=new Date(k),e=new Date;$=Math.max(1,Math.round((e.getTime()-t.getTime())/864e5)+1)}const y=140,w=b.length>=2?$t(b,460,y):"",z=_.length>=2?`${_[0].date} → ${_[_.length-1].date}`:"";return V`
      <div class="col" style="gap:22px">
        ...
        <div class="segmented">
          ${At.map(t=>{const e=$>=Mt(t)||"7G"===t||0===$;return V`
              <button data-active="${this._range===t}"
                style="${e?"":"opacity:0.45"}"
                title="${e?"":`Servono almeno ${Mt(t)} giorni di storico (ne hai ${$})`}"
                @click=${()=>{this._range=t}}>
                ${t}
              </button>
            `})}
        ...
        <div class="card" style="padding:20px">
          ...
          ${w?V`
            ...
            <div class="text-mute text-xs" style="margin-top:8px;display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap">
              ...
              ${$<m?V`
                <span>Storico totale: ${$} giorni (richiesti ${m})</span>
              `:""}
            </div>
          `:V`
            <div style="height:200px;display:grid;place-items:center;color:var(--text-muted);text-align:center;padding:0 16px">
              ${0===u.length?V`Nessuno storico peso ancora.<br/><span class="text-xs">Lo storico cresce di una misurazione al giorno dalla bilancia Withings.</span>`:V`Nessun dato nel periodo selezionato.<br/><span class="text-xs">Hai ${$} giorni di storico totali.</span>`}
            </div>
          `}
        </div>
    ...
    `}_renderCardioHealth(){const t=this.card.monitorState,e=t?.resting_hr??[],i=t?.vo2max??[],s=t?.hrv_daily??[];if(0===e.length&&0===i.length&&0===s.length)return"";const r=200,a=40,o=e.slice(-30).map(t=>t.bpm),n=o.length>=2?$t(o,r,a):"",l=e.length>0?e[e.length-1].bpm:0,d=i.map(t=>t.value),c=d.length>=2?$t(d,r,a):"",p=i.length>0?i[i.length-1].value:0,h=s.slice(-30).map(t=>t.value_ms),v=h.length>=2?$t(h,r,a):"",g=s.length>0?s[s.length-1].value_ms:0;return V`
      <div>
      ...
      <text x="50" y="62" text-anchor="middle" fill="var(--text-muted)" font-size="10"
        ...
        <div class="card">
          ...
          <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:4px">
            ...
            `)}
            ${u.flatMap(t=>t.map(t=>{if(0===t.day)return V`<div></div>`;const e=t.intensity>=0?Pt[t.intensity]:"var(--bg-soft)",i=t.day===r.getDate();return V`
    ...
    `}_renderCorrelation(t,e){const i=t.filter(t=>t.volume_kg&&t.volume_kg>0).slice(-20);if(i.length<3||e.length<3)return V`
      ...
      `;const o=a.map(t=>t[0]),n=a.map(t=>t[1]),l=Math.min(...o),d=Math.max(...o),c=Math.min(...n),p=Math.max(...n),h=d-l||1,v=p-c||1,g=t=>s+(t-l)/h*220,u=t=>170-(t-c)/v*r,m=a.length,x=o.reduce((t,e)=>t+e,0)/m,f=n.reduce((t,e)=>t+e,0)/m;let _=0,b=0,k=0;for(const[t,e]of a)_+=(t-x)*(e-f),b+=(t-x)**2,k+=(e-f)**2;const $=b>0&&k>0?_/Math.sqrt(b*k):0,y=Math.abs($)>.6?"FORTE":Math.abs($)>.3?"MODERATA":"DEBOLE",w=b>0?_/b:0,z=f-w*x,C=g(l),A=u(w*l+z),M=g(d),S=u(w*d+z);return V`
      <div class="card">
        <div class="card__header">
          <div style="flex:1">
            <h3 class="card__title">Correlazione</h3>
          ...
          <span class="chip chip--ok">${y}</span>
        </div>
        <div class="sp-between" style="margin-bottom:10px">
          <span class="text-sm text-soft">Coefficiente r</span>
          <span class="mono fw-700" style="font-size:22px;color:${$<0?"var(--ok)":"var(--warn)"}">${$.toFixed(2)}</span>
        </div>
        <svg viewBox="0 0 ${280} ${200}" width="100%" style="display:block;max-height:200px">
          ...
          `)}
          <line x1="${C}" y1="${A}" x2="${M}" y2="${S}" stroke="var(--danger)" stroke-width="1.5" stroke-dasharray="5 3" opacity="0.6"/>
        </svg>
      </div>
      ...
      <aside class="sidebar" data-mode="${t}">
        ...
        <div class="sidebar__brand">
          ...
          ${e?G:V`<div>
                ...
                <div class="sidebar__brand-sub">v${"1.5.0"}</div>
              </div>`}
        </div>
